#pragma once

// Generated by dougthedruid on Discord
// https://github.com/DougTheDruid

// Size 0x50
struct ObjectMessagingObjectScoper
{
public:
	char                                                         pad0x50_FWRK2[0x50];                               // 0x0(0x50)
};


// Size 0x58
struct ObjectMessagingHandle
{
public:
	char                                                         pad0x8_J4VRY[0x8];                                 // 0x0(0x8)
	class Struct*                                                EventType;                                         // 0x8(0x8)
	char                                                         pad0x48_3DYRE[0x48];                               // 0x10(0x48)
};


// Size 0x8
struct EventObjectMessagingDispatcherBeingDestroyed
{
public:
	char                                                         pad0x8_B5F0U[0x8];                                 // 0x0(0x8)
};


// Size 0x8
struct ObjectMessagingDispatcherHandle
{
public:
	char                                                         pad0x8_9BM1H[0x8];                                 // 0x0(0x8)
};


// Size 0xa0
struct ObjectMessagingDispatcher
{
public:
	char                                                         pad0xa0_EOJJ3[0xa0];                               // 0x0(0xa0)
};


