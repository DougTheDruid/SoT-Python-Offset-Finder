// Generated by DougTheDruid#2784 on Discord
// https://github.com/DougTheDruid
namespace DougsSDKDumper
{
//-----
// Classes
//-----

// Size 0x18
class BP_LargeShip_CageLatch_03_a_C: public PetCageSwitchMechanismTrigger
{
public:
	Struct PointerToUberGraphFrame                               UberGraphFrame;                                    // 0x580(0x8)
	Class StaticMeshComponent*                                   cap_cage_door_03_a;                                // 0x588(0x8)
	Class InteractableComponent*                                 Interactable;                                      // 0x590(0x8)
};


// Size 0x18
class BP_LargeShip_CageLatch_03_b_C: public PetCageSwitchMechanismTrigger
{
public:
	Struct PointerToUberGraphFrame                               UberGraphFrame;                                    // 0x580(0x8)
	Class StaticMeshComponent*                                   cap_cage_door_03_b;                                // 0x588(0x8)
	Class InteractableComponent*                                 Interactable;                                      // 0x590(0x8)
};


}