...
search_dir = Path('SDK')  # You can redefine SDK path.
config_file = Path('find.yaml')
...
def get_sdk_file_offsets(filename):
    ...
    if file_path is None:
        log.warning(f'Cannot find {file_path}')
        return {}
    ...
    for class_anchor, fields_config in to_find.items():
        ...
        class_text = re.findall(re.escape(class_anchor) + class_find_pattern, file_text)
        if not class_text:
